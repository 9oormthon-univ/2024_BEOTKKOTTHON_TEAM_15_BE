...
public class MemberTeamServiceImpl implements MemberTeamService {
    ...
    @Transactional
    @Override
    public MemberListResponseDto deleteMemberTeam(Long teamId, Long memberId) {  // 그룹의 멤버 탈퇴 (MEMBER 및 LEADER 본인 탈퇴는 가능. CREATOR 본인 탈퇴는 불가능. 멤버 탈퇴는 LEADER와 CREATOR만 가능. 리더 탈퇴는 CREATOR만 가능.)
        // 탈퇴하고자하는 사용자
        ...
        if(memberId == loginMemberId) {  // 본인 탈퇴인 경우
            ...
            if(loginRole == Role.CREATOR) {
                ...
                if(outRole == Role.LEADER || outRole == Role.CREATOR) {
                    throw new GeneralException(ErrorStatus.BAD_REQUEST, "그룹 탈퇴 ERROR - LEADER는 LEADER or CREATOR인 타인 퇴출 불가능");
                }
                else {
                ...
                throw new GeneralException(ErrorStatus.BAD_REQUEST, "그룹 탈퇴 ERROR - MEMBER는 타인 퇴출 불가능");
            }
        }

        return memberService.findMembersByTeam(teamId);
    }
}
